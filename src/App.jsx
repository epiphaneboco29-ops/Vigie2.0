...
function ChatPane({ mode, apiCall }) {
  ...
  async function send() {
    ...
    if (attachment?.isImage) {
      ...
      const composed =
        (text || "Voici un document à examiner.") +
        ...
        {messages.map((m, i) => (
          <div key={i} className={"flex " + (m.role === "user" ? "justify-end" : "justify-start")}>
            <div className={"max-w-[85%] flex flex-col gap-1.5 " + (m.role === "user" ? "items-end" : "items-start")}>
              <div
                className={
                  "rounded-sm px-4 py-3 text-[14.5px] leading-relaxed whitespace-pre-wrap " +
                  (m.role === "user"
      ...
      <div className="border-t border-[#D8CFB8] pt-3 flex flex-col gap-2">
        ...
        <div className="flex gap-2 items-end">
          ...
          <button
            ...
            className={
              "shrink-0 rounded-sm p-2.5 transition-colors border " +
              (recording
...
function AnalysePane({ apiCall }) {
  ...
  async function analyse() {
    ...
    try {
      ...
      const prompt =
        "Voici des données de terrain (CSV, " + rows.length + " lignes au total, échantillon ci-dessous en JSON) :\n" +
        JSON.stringify(sample) +
        "\n\nQuestion de l'utilisateur : " +
        (question || "Fais une lecture épidémiologique générale de ces données (tendance, type de courbe, points d'attention).");
